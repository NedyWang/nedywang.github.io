...
digraph A {
	...
	event_io_map [ label="{<head>...|<evmap_io>evmap_io|...}" ];

	evmap_io[label="{<events>events|nread|nwrite|nclose}"]

	edge [fontname = "Verdana", fontsize = 10, color="crimson", style="solid"]
	event_io_map:evmap_io -> evmap_io:events

	events [label="{<head>fd|callback|next|...}"]
	evmap_io:events->events

	events->events:head [style="dashed", color="forestgreen"];
}
```
